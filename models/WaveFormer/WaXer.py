...
# ==============================================================================
# 1. Basic Layers & Embeddings (Dependencies)
# ==============================================================================
...
class WaveNetForecaster(nn.Module):
    ...
    def forward(self, x_enc, return_feature=False):
        # 1. Normalization (RevIN part inside WaveNet)
        ...
        # Feature Extraction Mode: Return the rich temporal features
        # Shape: [B, d_model, T]
        if return_feature:
            return skip_connections

        return skip_connections # Default return (unused in TimeXer flow but kept for structure)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, cross):
        ...
        x_glb_ori = x[:, -1, :].unsqueeze(1) # Global Token
...
# ==============================================================================
# 4. Final Integrated Model (TimeXer + WaveNet)
# ==============================================================================

class TimeXerWithWaveNet(nn.Module):
    def __init__(self,
                 # TimeXer Params
                 seq_len=96,
                 pred_len=96,
        ...
        self.wavenet = WaveNetForecaster(
            seq_len=seq_len,
            pred_len=pred_len,
            c_in=wavenet_c_in,
    ...
    def forward(self, x_enc, x_mark_enc=None):
        ...
        # --- A. WaveNet Feature Extraction (Using Raw Data) ---
        # WaveNet internally handles normalization, so we pass raw x_enc.
        # Output: [B, d_wave, T]
        ...
        # en_embed: [Batch * N_vars, Patch_Num + 1, d_model]
        en_embed, _ = self.en_embedding(x_enc.permute(0, 2, 1))

        # ex_embed Broadcasting:
        # WaveNet features are global (mixed vars), but TimeXer treats vars individually in batch dim.
        # We repeat the global WaveNet context for each variable.
        # [B, T, D] -> [B, 1, T, D] -> [B, N_vars, T, D]
        ex_embed = ex_embed.unsqueeze(1).repeat(1, n_vars, 1, 1)
        
        # Flatten to [Batch * N_vars, T, D] to match en_embed's batch dimension
        ex_embed = ex_embed.reshape(batch_size * n_vars, seq_len, -1)

        # --- D. Encoder ---
        # Query: en_embed (Patches)
        # Key/Value: ex_embed (WaveNet Features)
        enc_out = self.encoder(en_embed, ex_embed)

        # --- E. Decoding & De-normalization ---
        # Reshape back to [B, N, D, P]
